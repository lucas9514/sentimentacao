...
def analisar_comentarios(df, coluna_comentario, modelo):
    ...
    # 🔥 LISTAS SIMPLES DE AJUSTE
    palavras_positivas = [
        "amei", "lindo", "lindos", "perfeito", "maravilhoso", "incrível",
        "incrivel", "top", "bom", "ótimo", "otimo", "sensacional",
        "parabéns", "parabens", "amo", "amei", "👏", "❤️", "😍"
    ]

    palavras_negativas = [
        "péssimo", "pessimo", "horrível", "horrivel", "ruim", "lixo",
        "não gostei", "nao gostei", "odiei", "terrível", "terrivel",
        "decepção", "decepcao", "absurdo", "ridículo", "ridiculo"
    ]
    ...
    for i, comentario in enumerate(comentarios, start=1):
        ...
        # 🔥 AJUSTE INTELIGENTE (override simples)
        if any(p in comentario_lower for p in palavras_positivas):
            sentimento = "Positivo/Neutro"

        if any(p in comentario_lower for p in palavras_negativas):
            sentimento = "Negativo"
...
def preparar_top_negativos(df, quantidade_top=4):
    ...
    # 1) CSV local antigo: pode ter coluna "confianca"
    # 2) Backend novo: devolve apenas comentario, sentimento e label_modelo
    #
    # Como removemos confiança/score da experiência FHITS, o ranking agora usa
    # uma heurística simples: termos críticos + tamanho do comentário.
    negativos["score_relevancia"] = negativos.apply(
        lambda row: (
            min(len(str(row["comentario"])) / 200, 1) * 0.5
            + min(row["termos_criticos"], 3) * 0.5
        ),
        axis=1
    )

    negativos = negativos.sort_values(
        by=["score_relevancia", "termos_criticos"],
        ascending=False
...
def buscar_comentarios_apify(lista_links, plataforma, limite_comentarios=APIFY_RESULTS_LIMIT):
    ...
    url_api = f"https://api.apify.com/v2/acts/{APIFY_ACTOR_ID}/run-sync-get-dataset-items"

    payload = {
        "directUrls": lista_links,
    ...
    try:
        ...
        for item in dados:
            ...
            if texto:
                comentarios.append({
                    ...
                    "curtidas_comentario": item.get("likesCount", 0),
                    "url_publicacao": item.get("postUrl", ""),
                    "url_comentario": item.get("commentUrl", ""),
        ...
        st.error("A consulta na Apify demorou demais. Tente reduzir a quantidade de comentários ou testar apenas 1 link.")
        return None

    except requests.exceptions.RequestException as e:
        st.error(f"Erro ao consultar a Apify: {e}")
        return None

    except Exception as e:
        st.error(f"Erro ao processar resposta da Apify: {e}")
        return None


def anexar_metadados_apify(resultado_df, df_comentarios):
    if resultado_df.empty or df_comentarios.empty:
        return resultado_df
    ...
    df_metadados = df_comentarios[colunas_metadados].reset_index(drop=True)
    resultado_df = resultado_df.reset_index(drop=True)

    for coluna in colunas_metadados:
        resultado_df[coluna] = df_metadados[coluna].values[:len(resultado_df)]
...
def analisar_link_backend(url, plataforma):
    status = st.empty()
    progresso = st.progress(0, text="Preparando análise da publicação...")

    status.markdown(
        '<div class="fake-status-box">🔗 Enviando link para a Apify...</div>',
        unsafe_allow_html=True
    )
    progresso.progress(0.25, text="Buscando comentários reais na Apify...")

    df_comentarios = buscar_comentarios_apify(
        lista_links=[url],
        plataforma=plataforma,
        limite_comentarios=APIFY_RESULTS_LIMIT
    )
    ...
    status.markdown(
        '<div class="fake-status-box">🤖 Classificando sentimentos dos comentários reais...</div>',
        unsafe_allow_html=True
    )
    progresso.progress(0.70, text="Classificando sentimentos...")

    inicio_total = time.time()

    with st.spinner("Carregando modelo de sentimento..."):
        modelo = carregar_modelo()

    resultado_df, tempo_modelo, tempo_medio = analisar_comentarios(
        df_comentarios,
        "comentario",
        modelo
    )
    ...
    resultado_df = anexar_metadados_apify(resultado_df, df_comentarios)

    progresso.progress(1.0, text="Publicação analisada com sucesso.")

    payload = {
        "modo": "link_apify",
        "url": url,
        "plataforma": plataforma,
        "total_comentarios": len(resultado_df),
        "tempo_total": tempo_total,
        "tempo_medio": tempo_medio
    }

    return resultado_df, payload
...
def analisar_multiplos_links_backend(lista_links, plataforma):
    ...
    if df_comentarios.empty:
        payload = {
            "modo": "multiplos_links_apify",
            "plataforma": plataforma,
            "total_links": len(lista_links),
            "total_comentarios": 0
        }
        return pd.DataFrame(columns=["comentario", "label_modelo", "sentimento"]), payload
    ...
    with st.spinner("Classificando sentimentos dos comentários reais..."):
        modelo = carregar_modelo()
        resultado_df, tempo_modelo, tempo_medio = analisar_comentarios(
            df_comentarios,
            "comentario",
    ...
    with col_right:
        if modo_entrada == "CSV":
            ...
            if iniciar_link:
                ...
                with st.spinner("Consultando a Apify e analisando a publicação..."):
                    inicio_apify = time.time()
                    resultado_df, resposta_backend = analisar_link_backend(
                        url=url_publicacao,
                        plataforma=st.session_state.get("plataforma_cliente", "Instagram")
                    )
                    tempo_total = time.time() - inicio_apify
